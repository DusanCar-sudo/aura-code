...
/** One member of a task's swarm. Status is written by the engine as it runs. */
...
export interface SwarmDef {
  strategy?: 'parallel' | 'pipeline' | 'hierarchical';
  agents: SwarmAgent[];
}

/**
 * Validate a swarm arriving from a client, on the same terms as a workflow:
 * it persists to the board file and is read back on every load, so a roster
 * without agents (or with unnamed ones) is rejected at the wire instead of
 * breaking the board later.
 */
export function isSwarmDef(v: unknown): v is SwarmDef {
  if (!v || typeof v !== 'object') return false;
  const d = v as { strategy?: unknown; agents?: unknown };
  if (d.strategy !== undefined && d.strategy !== 'parallel' && d.strategy !== 'pipeline' && d.strategy !== 'hierarchical') return false;
  if (!Array.isArray(d.agents) || d.agents.length === 0) return false;
...
export interface BoardTask {
  ...
  failed?: boolean;
  /**
   * A multi-agent swarm attached to this task.
   *
   * Persisted so a launch from the web client survives a reload, and so the
   * engine can actually run it: `board.run` on a task carrying more than one
   * agent executes the swarm rather than a single turn.
   */
  swarm?: SwarmDef;
